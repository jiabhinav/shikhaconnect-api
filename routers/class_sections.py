...
def school_storage(school_id: int, session_id: int, db: Session = Depends(get_db_session),
...
def record(db, model, school_id, session_id, record_id):
    item = db.query(model).filter_by(school_id=school_id, session_id=session_id, id=record_id).first()
    if item is None:
...
def save(db, item):
    try:
        ...
        db.rollback()
        raise HTTPException(409, "Name already exists for this school session or the record conflicts with database constraints") from exc
    except Exception:
        db.rollback()
        raise
...
def create_class(school_id: int, session_id: int, payload: ClassWrite, db: Session = Depends(school_storage)):
    ...
    if order is None:
        order = (db.query(func.max(SchoolClass.class_order)).filter_by(school_id=school_id, session_id=session_id).scalar() or 0) + 1
