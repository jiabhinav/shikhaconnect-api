...
def fee_category_school(school_id: int, session_id: int, db: Session = Depends(get_db_session),
...
def find_fee_category(db, school_id, session_id, fee_category_id):
    item = db.query(FeeCategory).filter_by(school_id=school_id, session_id=session_id, id=fee_category_id).first()
    if item is None:
...
def save_fee_category(db, item, message):
    try:
        ...
        db.rollback()
        raise HTTPException(409, "Fee category name already exists for this school session or conflicts with database constraints") from exc
    except Exception:
        db.rollback()
        raise
