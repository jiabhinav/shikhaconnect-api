...
def house_school(school_id: int, session_id: int, db: Session = Depends(get_db_session),
...
def find_house(db, school_id, session_id, house_id):
    item = db.query(House).filter_by(school_id=school_id, session_id=session_id, id=house_id).first()
    if item is None:
...
def save_house(db, item, message):
    try:
        ...
        db.rollback()
        raise HTTPException(409, "House name already exists for this school session or conflicts with database constraints") from exc
    except Exception:
        db.rollback()
        raise
