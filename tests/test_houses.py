...
class HouseTests(unittest.TestCase):
    ...
    def test_crud_and_duplicates(self):
        url = "/schools/school/1/sessions/1/houses"
        ...
        self.assertEqual(self.client.get(f"/schools/school/2/sessions/2/houses/{item['id']}").status_code, 404)
    ...
    def test_admin_sub_admin_and_school_scope(self):
        ...
        for role in (UserRole.ADMIN, UserRole.SUB_ADMIN):
            self.user.role = role
            self.assertEqual(self.client.post(url, json={"name": "EMERALD"}).status_code, 403)
    ...
    def test_auto_creation_through_database_dependency(self):
        ...
        with patch.object(database, "engine", self.engine), patch.object(database, "SessionLocal", lambda: database.sessionmaker(bind=self.engine)()):
            response = self.client.post("/schools/school/1/sessions/1/houses", json={"name": "EMERALD"})
        self.assertEqual(response.status_code, 201, response.text)
    ...
    def test_validation_and_same_name_other_school(self):
        ...
        for school_id in (1, 2):
            self.assertEqual(self.client.post(f"/schools/school/{school_id}/sessions/{school_id}/houses", json={"name": "EMERALD"}).status_code, 201)
